...
const PostCard = ({ post, startEditing = false, showActions = false }) => {
  ...
  const savedFromAuth = Boolean(
    authUser?.savedPosts?.some((id) => (id?._id || id).toString() === post._id),
  );
  ...
  const commentMutation = useMutation({
    mutationFn: () => createComment({ postId: post._id, content: commentText }),
    onSuccess: () => {
      setCommentText("");
  ...
  const liked = post.likes?.some(
    (id) => (id?._id || id).toString() === authUser?._id,
  );
  const isOwner =
    post.author?._id?.toString() === authUser?._id?.toString() ||
    post.author?.toString() === authUser?._id?.toString();

  return (
    <article className="card border border-base-300 bg-base-100 shadow-sm overflow-hidden">
      <div className="flex items-center gap-3 p-4">
        <div className="avatar">
          <div className="w-10 rounded-full bg-primary/10">
            <img src={post.author?.profilePic || "/avatar.png"} alt="" />
          </div>
        </div>
        <div className="flex-1">
          <p className="font-bold">{post.author?.fullName}</p>
          <p className="text-xs opacity-60">
            @{post.author?.username || "connecter"} ·{" "}
        ...
        {showActions && isOwner && (
          ...
          >
            ...
            {menuOpen && (
              <ul className="dropdown-content menu z-20 mt-2 w-44 rounded-box border border-base-300 bg-base-100 p-2 shadow-xl">
                <li>
                  <button
                    onClick={() => {
                      setMenuOpen(false);
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (
                        ...
                        postDeleteMutation.mutate(post._id);
                    }}
                  >
      ...
      {post.mediaType === "video" ? (
        <video
          src={post.mediaUrl}
          controls
          className="max-h-[620px] w-full bg-base-300 object-contain"
        />
      ) : (
        <img
          src={post.mediaUrl}
      ...
      <div className="p-4">
        {editingPost && (
          <form
            onSubmit={(event) => {
              ...
              postEditMutation.mutate({ postId: post._id, caption });
            }}
            className="mb-4 rounded-2xl border border-primary/20 bg-primary/5 p-3"
          >
            <div className="mb-2 flex items-center justify-between">
              <p className="text-sm font-bold">Edit your caption</p>
              <button
                type="button"
            ...
            <textarea
              ...
              maxLength={500}
              className="textarea textarea-bordered min-h-20 w-full bg-base-100"
              placeholder="Write a caption..."
            />
            <div className="mt-2 flex items-center justify-between">
              <span className="text-xs opacity-50">{caption.length}/500</span>
              <button
                className="btn btn-primary btn-sm gap-2"
                disabled={postEditMutation.isPending}
              >
                <Check size={15} />
                {postEditMutation.isPending ? "Saving..." : "Save caption"}
              </button>
            </div>
          </form>
        )}
        <div className="flex items-center gap-1">
          ...
          >
            <Heart size={21} className={liked ? "fill-error text-error" : ""} />
          </button>
          <button
            onClick={() => setShowComments(!showComments)}
            className={`btn btn-ghost btn-circle btn-sm ${showComments ? "text-primary" : ""}`}
          >
              ...
              {commentsLoading ? (
                ...
                comments.map((comment) => {
                  ...
                  return (
                    <div
                      key={comment._id}
                      className="group rounded-xl px-2 py-1 text-sm hover:bg-base-200/60"
                    >
                      <div className="flex items-start gap-2">
                        ...
                        {ownsComment && editingCommentId !== comment._id && (
                          <button
                            aria-label="Edit comment"
                            className="btn btn-ghost btn-circle btn-xs opacity-0 transition group-hover:opacity-100"
                            onClick={() => {
